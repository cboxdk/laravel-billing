...
| **Metering / event log** | `EventLog` contract = the immutable metering source of truth (append idempotent by event id · sum for invoice computation). **Storage is optional/pluggable**: `InMemoryEventLog` (default) · `DatabaseEventLog` (MySQL/Postgres/sqlite — plenty for small deployments) · a ClickHouse adapter drops in behind the same contract for event-heavy scale (**ClickHouse is optional, not required**). `DefaultMeterIngest` appends to the log; config `billing.metering.event_log = memory\|database`. | ✅ in-memory + relational (dedup + windowed sum) + idempotent ingest tests |
